...
- **Keybinds:**
  ...
  - `<leader>\h` / `<leader>\j` / `<leader>\k` / `<leader>\l` — move (and show) the terminal to left/bottom/top/right respectively, updating the remembered side/size for that worktree. Deliberately kept off the `<M-\>` prefix (unlike the `<M-b>` / `<M-b>b` / `<M-b>w` precedent in `lua/config/telescope/git_branch.lua`) so the bare `<M-\>` toggle is not a prefix of any other mapping and fires immediately, instead of waiting out `timeoutlen` (unset in this config, so Neovim's 1000ms default) on every toggle press. `<leader>\` itself is left unbound — only its `h`/`j`/`k`/`l` children exist, so no new prefix ambiguity is introduced in the `<leader>` space.
...
- **2026-09-11 revision:** the directional mount keybinds were moved from `<M-\>h/j/k/l` to `<leader>\h/j/k/l` after the user noticed the bare `<M-\>` toggle felt slow in practice — it was waiting out the unset (1000ms default) `timeoutlen` to disambiguate from the chorded variants. `<leader>t` was ruled out as the new prefix since it's already used for opening a new tab. `<M-\>` itself is unchanged and is no longer a prefix of any other mapping.
